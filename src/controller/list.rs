...
pub fn list_running(file_name: &str) -> Result<()> {
 ...
 let running_activities = getter::get_running_activities(&file_content);list::list_running_activities(&running_activities);
...
) -> Result<()> { let file_content = bartib_file::get_file_content(file_name)?;
  
 let activities = getter::get_activities(&file_content).collect(); let processed_activities_bind: Vec<activity::Activity> =
...
filtered_activities.sort_by_key(|activity| activity.start);
 ...
 ); if do_group_activities && !filter.date.is_some() {list::list_activities_grouped_by_date(&filtered_activities[first_element..]);} else { 
...
let mut has_finding: bool = false; let mut last_end: Option<NaiveDateTime> = None;
...
has_finding = !check_sanity(last_end, &activity, line_number) || has_finding;
    ...
    if let Some(e) = last_end {
      ...
      if this_end > e { last_end = Some(this_end); }
        }
  } else { last_end = activity.end; }
...
fn check_sanity(
 ...
 let mut sane = true;
    ...
    if activity.get_duration().num_milliseconds() < 0 { println!("Activity has negative duration");
sane = false;
 }
 if let Some(e) = last_end {
        if e > activity.start {
